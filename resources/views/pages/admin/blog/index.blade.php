...
@section('content')
    <div class="table-agile-info">
                    ...
                    <tbody class="list-content">
                        @foreach ($getAllBlog as $key => $blog)
                            <tr>
                                ...
                                <td class="management">
                                    <button type="button" onclick="updateContent('update', {{ $blog->id }})"
                                        class="management-btn" title="@lang('vart_define.button.update')">
                                        <i class="fa fa-pencil-square-o text-success text-active"></i>
                                    </button>
                                    <button onclick="deleteContent({{ $blog->id }})" class="management-btn"
        ...
        {{ $getAllBlog->links('pagination::custom') }}
    </div>
    <div id="portal"></div>
@endsection
@push('js')
    ...
    <script type="text/javascript">
        ...
        var url_load_content = "{{ route('blog.load') }}";
        var url_get_form = "{{ route('blog.get_form') }}";
        var url_create_or_update = "{{ route('blog.store_or_update') }}";
        var url_delete = "{{ route('blog.destroy') }}";
